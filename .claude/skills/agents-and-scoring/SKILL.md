...
├──────────────────┼──────────┼──────────────────────────┼──────────────┤
│ SocialAgent      │ 10%      │ ⚠️ なし（常時中立固定）  │ -1.0 〜 +1.0 │
├──────────────────┼──────────┼──────────────────────────┼──────────────┤
...
### SocialAgent（⚠️ 常時ニュートラル固定 — 実データ未取得、2026-07-08訂正）
...
**現状（HEAD、2026-05-24以降 約1.5ヶ月継続）**: `skills/social_monitor.py`
の `fetch_social_sentiment()` は `SOCIAL_USE_MOCK=false`（本番デフォルト）
の場合、外部通信を一切行わず **`sentiment=NEUTRAL, hype_score=0.0, score=0.0`
を無条件で返す**。コミット `fb07c18`（2026-05-24、「Ollama LLM 3バグ修正」）
で「モックバイアスの加重スコアへの混入を防ぐ安全側設定」として意図的に
導入された。

**重要な事実（研究データ解釈に必須）**: SocialAgentのシグナルは
**2026-05-01の初回実装以降、一度も実際のSNSデータを反映したことがない**。
...
- 2026-05-24〜現在: 上記の通り無条件でNEUTRAL固定。LLM呼び出しすら発生しない。
- つまり `data/research/hold_cases.jsonl` 等、これまでに蓄積された研究データの
  SocialAgentシグナルは、**どの期間のものであっても実世界のセンチメントを
  表していない**。過去の分析・ブログ記事等でSocialAgentの寄与を解釈する際は
  この制約を踏まえること。

**検討中・未検証の実装（作業ツリーに存在、2026-07-08時点で未コミット）**:
StockTwits公開API（認証不要のエンドポイント）から実投稿を取得する実装が
`skills/social_monitor.py` に書かれているが、専用テストが無く、実際に
APIから値が返ることを確認した記録も無い。**「StockTwitsへの回帰を提案
しない」という過去の誤った注記は撤回する** — StockTwits公開APIは現時点で
唯一の実装候補であり、回避すべき対象ではない。ただし本番投入するには
動作確認（`--dry-run`での疎通確認、レスポンス形式の検証）が必要であり、
未検証のままコミット・有効化しないこと。

旧仕様の記述（閾値 `SOCIAL_HYPE_THRESHOLD = 0.7` 以上を「過熱相場」と
判定する等のロジック自体）はコードとして現存し、直下の買い煽りペナルティ
節に記載の通り正しい。ただし**入力が常にNEUTRAL固定のため、このペナルティ
分岐は2026-05-24以降、事実上発火していない**（後述）。
...
>
> **現在の実効性**: 中立スタブは `"score": 0.0` を明示的に返すため、
> Managerは常に**連続スコアモード**（`"score" in social_data` が真）を
> 通り、後方互換モードの買い煽りペナルティ分岐そのものに到達しない。
> ロジックは実装済みだが、2026-05-24以降**実質的に発火していない**。
